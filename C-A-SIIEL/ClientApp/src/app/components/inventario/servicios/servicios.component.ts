...
export class ServiciosComponent implements OnInit {
  ...
  EmplArray: Titulos[] = [
    { id: 1, title: "Nombre1 Registro Tabla", descripcionT: "Descripción1 Regitro Tabla", done: false, createdDate: this.dateString1, updatedDate: "2000-01-11" },
  ...
  AddEditTodo() {
    // NO hay empleado seleccionado == 0, es nuevo empleado
    if (this.sel.id === 0) {
      this.sel.id = this.ServArray.length + 1;
      ...
      this.sel.bodega = this.formT.controls['bodega'].value,
        ...
        this.ServArray.push(this.sel);
    }
    else {
      this.ServArray.push(this.sel);
      this.ServArray = this.ServArray.filter(x => x != this.sel);
